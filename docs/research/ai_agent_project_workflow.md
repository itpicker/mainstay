...
고객(User)으로부터 프로젝트나 Task에 대한 최초 요청을 받는 단계입니다. 이 단계에서는 **자유 양식의 의뢰**와 **필수 정보 수집(Pre-screening)** 방식이 혼용될 수 있습니다.

#### 📋 필수 정보 사전 질의 (Mandatory Intake Strategy)
사용자의 의뢰가 너무 막연할 경우, AI는 프로젝트 착수에 필요한 최소한의 정보를 확보하기 위해 **'사전 질의서(Intake Form)'** 모드로 전환하거나, 필수 항목을 능동적으로 되물어야 합니다.

**[필수 수집 항목 5가지]**
프로젝트의 성격을 규정하고 담당 에이전트를 배정하기 위해 반드시 확인해야 하는 항목입니다.

| 항목 | 질문 내용 (Example) | 수집 목적 |
| :--- | :--- | :--- |
| **1. 목적 (Purpose)** | "이 프로젝트를 통해 달성하고자 하는 핵심 목표는 무엇인가요?" | 프로젝트의 성격(구축/운영/고도화) 파악 |
| **2. 대상 (Target)** | "이 서비스/기능의 주 사용자는 누구인가요?" | UX/UI 설계 및 기능 범위 산정의 기준 |
| **3. 범위 (Scope)** | "핵심 기능 1~3가지만 꼽는다면 무엇인가요? (MVP 기준)" | 초기 리소스 산정 및 불필요한 스펙 제거 |
| **4. 예산/일정 (Constraints)** | "생각하시는 예산 범위나 반드시 지켜야 할 마감일이 있나요?" | 현실적인 구현 가능성 및 투입 레벨 판단 |
| **5. 레퍼런스 (Reference)** | "참고하고 계신 유사 서비스나 경쟁사가 있나요?" | 구체적인 눈높이(Expectation) 확인 |

**[접수 프로세스 로직]**
1.  **User Input:** 사용자가 의뢰 내용 입력 (Text)
2.  **AI Analysis:** 입력 내용 분석하여 '필수 항목 5가지' 중 누락된 정보 식별
3.  **Branching:**
    *   *Case A (정보 충분):* 즉시 Phase 2(요구사항 구체화)로 진입하여 PRD 초안 작성
    *   *Case B (정보 부족):* "원활한 진행을 위해 다음 3가지 사항을 더 알려주시겠습니까?"라고 역질문 (Missing Info 요청)
